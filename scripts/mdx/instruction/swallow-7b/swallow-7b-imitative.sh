...
JOB_ID=$(date +%s%N)
export MASTER_ADDR=10.130.184.16
export MASTER_PORT=12816
...
CHECKPOINT_SAVE_DIR="/model/fujii/checkpoints/Swallow-7b/baseline-imitative-lr_${LR}-minlr_${MIN_LR}"

mkdir -p ${CHECKPOINT_SAVE_DIR}

# data config

DATASET_DIR=/model/fujii/datasets/instruction/swallow/instruction/baseline-imitative

TRAIN_DATA_PATH=${DATASET_DIR}/train.jsonl
VALID_DATA_PATH=${DATASET_DIR}/val.jsonl

# job name
JOB_NAME="Swallow-7b-VE-baseline-imitative-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}"
