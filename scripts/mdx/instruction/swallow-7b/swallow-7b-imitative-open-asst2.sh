...
JOB_ID=$(date +%s%N)
export MASTER_ADDR=10.130.184.21
export MASTER_PORT=12821
...
CHECKPOINT_SAVE_DIR="/model/fujii/checkpoints/Swallow-7b/imitative-open-asst2-lr_${LR}-minlr_${MIN_LR}"

mkdir -p ${CHECKPOINT_SAVE_DIR}

# data config

DATASET_DIR=/model/fujii/datasets/instruction/swallow/instruction/imitative-open-asst2

TRAIN_DATA_PATH=${DATASET_DIR}/train.jsonl
VALID_DATA_PATH=${DATASET_DIR}/val.jsonl

# job name
JOB_NAME="Swallow-7b-VE-imitative-open-asst2-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}"
